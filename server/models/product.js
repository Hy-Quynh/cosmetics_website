...
module.exports = {
  ...
  getProductQuantity: async (productId) => {
    try {
      const response = await mysql.query(
        `SELECT currrent_quantity FROM product WHERE _id=${Number(productId)}`
      );
      if (response) {
        return response?.[0]?.currrent_quantity;
      }
      return 0;
    } catch (error) {
  ...
  //           await mysql.query(`INSERT INTO product_checkout_detail(checkout_id, product_id, product_name, product_price, product_sale, product_quanlity, proudct_image, free_product) VALUES
  ...
  //     const addPromo = await mysql.query(
  ...
  //     if (addPromo?.rows) {
  //       const promoId =
  //         await mysql.query(`SELECT promo_id from product_promo ORDER BY created_day DESC
  //       LIMIT 1`);
  ...
  //   try {
  //     const keyWordRes = await mysql.query(`SELECT * FROM keyword_search`);
  ...
  //   try {
  //     const keyWordRes = await mysql.query(
  ...
  //         `SELECT * FROM product WHERE lower(unaccent(product_name)) LIKE '%${keyWordList?.[
  //           i
  //         ]?.keyword
  //           ?.toLowerCase()
  //           ?.normalize("NFD")
  //           .replace(/[\u0300-\u036f]/g, "")}%' ORDER BY product_id DESC`
  //       );
